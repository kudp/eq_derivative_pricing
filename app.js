...
function hullWhiteTheta(time, flatRate, meanReversion, volatility) {
  if (meanReversion <= 0) {
    return 0;
  }

  return meanReversion * flatRate + ((volatility ** 2) / (2 * meanReversion)) * (1 - Math.exp(-2 * meanReversion * time));
}

function discountFactor(rate, maturity) {
  return Math.exp(-rate * maturity);
}

function swapValue({ notional, maturity, fixedRate, side, curveRate }) {
  const remainingPayments = Math.max(1, Math.round(maturity));
  let annuity = 0;

  for (let payment = 1; payment <= remainingPayments; payment += 1) {
    annuity += discountFactor(curveRate, payment);
  }

  const fixedLeg = fixedRate * annuity * notional;
  const floatingLeg = (1 - discountFactor(curveRate, remainingPayments)) * notional;
...
function priceCva(input) {
  const params = {
    ...
    flatRate: Number(input.flatRate),
  ...
  for (let path = 0; path < params.paths; path += 1) {
    let shortRate = params.flatRate;
    let pathDiscountFactor = 1;

    for (let step = 1; step <= totalSteps; step += 1) {
      const [z] = normalPair(random);
      const time = (step - 1) * dt;
      const theta = hullWhiteTheta(time, params.flatRate, params.meanReversion, params.volatility);
      shortRate += (theta - params.meanReversion * shortRate) * dt + params.volatility * Math.sqrt(dt) * z;
      shortRate = Math.max(-0.01, shortRate);
      pathDiscountFactor *= Math.exp(-shortRate * dt);
      ...
      discountedLoss += (1 - params.recoveryRate) * pathDiscountFactor * positiveExposure * defaultProbability;
    }
  }
  ...
  return {
    ...
    swapNpv: swapValue({
      ...
      fixedRate: params.fixedRate,
      side: params.side,
      curveRate: params.flatRate,
    }),
    curveDiscountFactor: discountFactor(params.flatRate, params.maturity),
    annualExposure,
    params,
  };
}
...
function renderResult(result) {
  ...
  document.querySelector("#curveLevel").textContent = percent(result.params.flatRate);
  ...
  const summary = [
    ...
    `Side=${sideLabel}`,
    `FlatCurve=${percent(result.params.flatRate)}`,
    `DiscountFactor=${result.curveDiscountFactor.toFixed(6)}`,
  ...
  document.querySelector("#calibrationLabel").textContent = `HW theta calibrated to flat ${percent(result.params.flatRate)} curve`;
...
if (typeof module !== "undefined") {
  module.exports = { priceCva, currency, swapValue, hullWhiteTheta, discountFactor };
}
